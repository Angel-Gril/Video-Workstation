...
export interface ExportPlanEntry {
  ...
  audioProcessing?: TimelineClip['audioProcessing']
  narration?: string
...
export function clipToPlanEntry(project: Project, clip: TimelineClip): ExportPlanEntry {
  ...
  if (track?.gain !== undefined) entry.trackGain = track.gain
  if (track?.fadeIn !== undefined) entry.trackFadeIn = track.fadeIn
  if (track?.fadeOut !== undefined) entry.trackFadeOut = track.fadeOut
  ...
  if (clip.audioProcessing) entry.audioProcessing = clip.audioProcessing
